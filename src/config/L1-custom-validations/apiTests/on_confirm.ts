...
async function validateFulfillments(
  ...
  for (const fulfillment of order.fulfillments || []) {
    ...
    if (fulfillment?.provider_id) {
      ...
      try {
        const isValidProviderId = await lookupSubscriber(
        ...
        if (isValidProviderId) {
          await RedisService.setKey(
            `${transaction_id}_provider_id_${ApiSequence.ON_CONFIRM}`,
            provider_id,
            TTL_IN_SECONDS
          );
        }
...
async function validateCreds(
  ...
  try {
    ...
    const credsRaw = await RedisService.getKey(
    ...
    if (!id || !code) {
      result.push({
        ...
        code: 30003,
        description:
          "order.creds.descriptor is missing id or code in /ON_SEARCH",
      });
      return;
    }
    ...
    if (!creds || !Array.isArray(creds)) {
      result.push({
        ...
        code: 30004,
        description:
          "Credentials array from Redis is missing or invalid in /ON_SEARCH",
      });
      return;
    }
    ...
    console.info(
      `Credential validation successful for id: ${id}, code: ${code}`
    );
  } catch (e: any) {
